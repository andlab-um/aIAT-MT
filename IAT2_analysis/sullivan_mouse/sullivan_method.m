...
shadedErrorBar(timeArray,abs(nanmean(l4.coef,2)), ...
    nanstd(l4.coef')'/sqrt(26), {'Color','g','linewidth',1.5},1);
...
shadedErrorBar(timeArray,abs(nanmean(l7.coef,2)), ...
    nanstd(l7.coef')'/sqrt(26), {'Color','r ','linewidth',1.5},1);
xlim([timeArray(1) timeArray(end)])
xlabel('Time normalized to 101 points','fontsize',24);
ylabel('Beta','fontsize',24)
set(gca,'XTick',0:20:100,'fontsize',20);
...
figure(2); hold on;
scatter(l4.st(1,:),zeros(1,26));
scatter(l7.st(1,:),ones(1,26));
...
RT = NaN(200,26);
for subj = 1:26
    thisSubjData = data4(data4(:,1)==(subj+133),:);
    for trial=1:200
    % find this subject
...
%logical variables
exclude = false(200,26);
reason.yCross =  false(200,26);
reason.RT =  false(200,26);
reason.overshoot =  false(200,26);
reason.smallRT =  false(200,26);
for subj = 1:26 % subject
