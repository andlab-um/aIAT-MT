...
##106,112: exclued
mt_ev1 <- mt_subset(mt_ev1,subjID!=106&subjID!=112)
...
##113,110,101: exclued
mt_ev2 <- mt_subset(mt_ev2,subjID!=113&subjID!=110&subjID!=101&subjID!=139&subjID!=141&subjID!=143&subjID!=145&subjID!=159)
...
MAD50<-mixed(MAD ~ (1+condition2.x|subjID)+condition2.x, data=results50)
AD50<-mixed(AD ~ (1+condition2.x|subjID)+condition2.x, data=results50)
AUC50<-mixed(AUC ~ (1+condition2.x|subjID)+condition2.x, data=results50)
MAD50
AD50
AUC50
...
av_tn_trajectories <- mt_aggregate_per_subject(mt_3,
                                               ...
                                               subject_id="subjID")
av_tn_trajectories1<-subset(av_tn_trajectories,subjID!=106&subjID!=112)
# Paired t-tests on coordinates
xpos_t_tests_ev1 <- 
  with(av_tn_trajectories1,
...
av_tn_trajectories <- mt_aggregate_per_subject(mt_3,
                                               use="tn_trajectories", use2_variables="condition2",
                                               subject_id="subjID")
av_tn_trajectories2<-subset(av_tn_trajectories,subjID!=110&subjID!=113&subjID!=101&subjID!=139&subjID!=141&subjID!=143&subjID!=145&subjID!=159)
# Paired t-tests on coordinates
xpos_t_tests_ev2 <- 
